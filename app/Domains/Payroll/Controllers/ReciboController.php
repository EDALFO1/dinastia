...
class ReciboController extends Controller
{
    ...
    // ðŸ”¥ SOLO RECIBOS DEL PERIODO ACTUAL
    ...
    // ðŸ”¥ PENDIENTES SOLO DEL PERIODO ACTUAL
...
public function store(Request $request)
{
    ...
    try {
        ...
        if($existe){
            return back()->with('error','Ya existe recibo para este perÃ­odo');
        }
...
public function usuariosSinRecibo()
{
    ...
    // ðŸ”¥ PERIODO ACTUAL (MES ANTERIOR)
    $periodo = now()->subMonth()->format('Y-m');
    ...
    // ðŸ”¥ RECIBOS DEL PERIODO
    $recibos = Recibo::where('empresa_id', $empresaId)
        ->whereRaw("DATE_FORMAT(DATE_SUB(fecha, INTERVAL 1 MONTH),'%Y-%m') = ?", [$periodo])
        ->pluck('afiliado_id');

    // ðŸ”¥ FILTRAR LOS QUE NO TIENEN RECIBO
...
public function cerrarPeriodo()
{
    ...
    // ðŸš¨ SI HAY PENDIENTES â†’ BLOQUEAR
    ...
    // âœ… SI TODO OK â†’ CERRAR PERIODO
    return redirect()->route('recibos.index')
        ->with('success', 'Periodo cerrado correctamente âœ”');
}
...
public function activosSiguientePeriodo()
{
    ...
    // =========================
    // ðŸ”¥ CONDICIÃ“N REUTILIZABLE (SIN RETIRO)
    // =========================
    ...
    // =========================
    // ðŸ”¥ 1. RECIBOS DEL MES SIN RETIRO
    // =========================
    ...
    // =========================
    // ðŸ”¥ 2. AFILIADOS QUE INGRESARON ESTE MES
    // =========================
...
public function crearLote(Request $request)
{
    ...
    if($recibos->isEmpty()){
        return back()->with('error','No hay recibos vÃ¡lidos para exportar');
    }
...
public function guardar($ruta)
{
    ...
    // TODO el cÃ³digo original exactamente igual
    // desde EMPRESA hasta el foreach

    // ðŸ”¥ IMPORTANTE:
    // NO cambies nada de las columnas
    // ni fÃ³rmulas del operador PILA
